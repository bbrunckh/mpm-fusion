...
# load data
mpm_val_national <- nanoparquet::read_parquet(here("output/fused_mpm_val_national.parquet"))
...
saveWorkbook(wb_summary, here("output/tables/mpm_summary_tables.xlsx"), overwrite = TRUE)
...
saveWorkbook(wb_extended, here("output/tables/mpm_extended_tables.xlsx"), overwrite = TRUE)
...
# scenario_colors <- c(
#   "p-c-r-e-w-s" = "#d73027",    # Red
#   "p-cr-ews" = "#fc8d59",       # Orange
#   "pcr-ews" = "#fee090",        # Light Yellow
#   "pcre-ws" = "#ffffbf",        # Pale Yellow (center)
#   "pce-rews" = "#e0f3f8",       # Light Blue
#   "pcew-rews" = "#91bfdb",      # Medium Blue
#   "pcews-rews" = "#4575b4"      # Dark Blue
# )
...
for (mpm_val in mpms) {
  for (metric in metric_cols) {
    ...
    ggsave(here("output", 
                paste0("figures/MPM/heatplot_", mpm_val, "_", metric, ".png")), 
...
for (mpm_val in mpms) {
  for (metric in metric_cols) {
    ...
    ggsave(here("output", 
                paste0("figures/MPM/heatplot_", mpm_val, "_", metric, "_samplebias.png")), 
           p, width = 12, height = 8, dpi = 300)
  }
}
...
for (est in estimates) {
  ...
  if (nrow(dt_plot) > 0) {
    ...
    filename <- paste0("figures/MPM/scatter_all_", 
                       est, ".png")
